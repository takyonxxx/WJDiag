#!/usr/bin/env python3
"""
WJ Diag - ELM327 Emulator v11 (APK-verified module map)
=========================================================
...
class KWP2000Responder:
    ...
    def _security(self, d):
        ...
        if d[0] == 0x02:
            if self.s.active_target == 0x20:
                ...
                return bytes([0x7F, 0x27, 0x35]) if len(d)==3 else bytes([0x7F, 0x27, 0x12])
    ...
    def _read_ecu_id(self, d):
        ...
        if self.s.active_target == 0x20:
            ...
            if o == 0x90: return bytes([0x7F, 0x1A, 0x33])
    ...
    def _ecu_block(self, b):
        ...
        if b in (0x60,0x62,0xB0,0xB1,0xB2): return bytes([0x7F, 0x21, 0x33])
        return bytes([0x7F, 0x21, 0x12])
...
class ELM327Server:
    ...
    async def run(self):
        ...
        log.info("=" * 60)
        log.info("  WJ Diag ELM327 Emulator v11 (APK-verified)")
